...
void leaf_node_split_insert(Cursor *cursor, uint32_t key, Row *value) {
    ...
    *(leaf_node_num_cells(new_node)) = LEAF_NODE_RIGHT_SPLIT_COUNT;
    if (is_node_root(old_node)) {
        ...
        exit(EXIT_FAILURE);
        /*
        uint32_t parent_page_num = *leaf_node_parent(old_node);
        uint32_t new_max_key = get_node_max_key(old_node);
        void *parent = get_page(cursor->table->pager, parent_page_num);
        update_internal_node_key(parent, get_node_max_key(old_node),
        new_max_key); return leaf_node_insert(cursor, new_max_key, new_node);
        */
    }
}
